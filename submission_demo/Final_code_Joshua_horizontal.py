...
if len(h_idx) <= 10: # Case 1 and case 2 work here
    ...
    for i in range(32):
        if i==0:
            ...
            mtx2outputdata(W1)
            mtx2outputdata(W2)
            Y_with_inference = read_blackbox()
            ...
            mtx2outputdata(W2)
            Y_with_inference = read_blackbox()
            a_plus_b_square = Y_with_inference[clean_idx, 0] # 可以换 column
            # Record 
...
#******************* Case 3 and case 4 work start at here ****************************
    ...
    for i in range(32):
        if i==0:
            ...
            a_square = estimated_real_Y_without_inference.T[30:30+N_tar, 1] # 可以换 column
            # record a_square into the first entry
            ...
            Y_with_inference = read_blackbox()
            estimated_real_Y_without_inference = np.asarray(estimate_real_Y_without_inference(W1, Y_with_inference))
            b_square = estimated_real_Y_without_inference.T[30:30+N_tar, 1] # 可以换 column
            # Compute a_plus_b_square
            W1 = np.mat(np.zeros((32, 32))) + 1j * np.mat(np.zeros((32, 32)))
            W1[0, 0] = 1/np.sqrt(2) + 1j * 0 # 可以换 column
            W1[i, 0] = 1/np.sqrt(2) + 1j * 0 # 可以换 column
            ...
            Y_with_inference = read_blackbox()
            estimated_real_Y_without_inference = np.asarray(estimate_real_Y_without_inference(W1, Y_with_inference))
            a_plus_b_square = estimated_real_Y_without_inference.T[30:30+N_tar, 1] # 可以换 column
            # Record 
            ab = a_plus_b_square - ( a_square / 2 ) - ( b_square / 2 )
    ...
    for i in range(32):
        if i==0:
            ...
            Y_with_inference = read_blackbox()
            estimated_real_Y_without_inference = np.asarray(estimate_real_Y_without_inference(W1, Y_with_inference))
            a_square = estimated_real_Y_without_inference.T[30:30+N_tar, 2] # 可以换 column
            # record a_square into the first entry
            ...
            Y_with_inference = read_blackbox()
            estimated_real_Y_without_inference = np.asarray(estimate_real_Y_without_inference(W1, Y_with_inference))
            b_square = estimated_real_Y_without_inference.T[30:30+N_tar, 2] # 可以换 column
            # Compute a_plus_b_square
            W1 = np.mat(np.zeros((32, 32))) + 1j * np.mat(np.zeros((32, 32)))
            W1[0, 0] = 1/np.sqrt(2) + 1j * 0 # 可以换 column
            W1[i, 0] = 1/np.sqrt(2) + 1j * 0 # 可以换 column
            ...
            Y_with_inference = read_blackbox()
            estimated_real_Y_without_inference = np.asarray(estimate_real_Y_without_inference(W1, Y_with_inference))
            a_plus_b_square = estimated_real_Y_without_inference.T[30:30+N_tar, 2] # 可以换 column
            # Record 
            ab = a_plus_b_square - ( a_square / 2 ) - ( b_square / 2 )
    ...
    Coordinate_Matrix = basis_vectors_pinv @ H1_matrix_row_space_estimator_2_basis.T
    # print(Coordinate_Matrix.shape) # Will get (10,300)
    
    # 从 coordinate matrix 拿回原本的 31 X 300 matrix
    sheet_new = H1_matrix_row_space_estimator_1.T @ Coordinate_Matrix
